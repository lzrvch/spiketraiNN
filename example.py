...
X_train = np.concatenate([crop_data['wake_train']['series'][indices, :], crop_data['sleep_train']['series'][indices, :]])
y_train = np.array([0] * indices.shape[0] + [1] * indices.shape[0])

X_test = np.concatenate([crop_data['wake_test']['series'][indices, :], crop_data['sleep_test']['series'][indices, :]])
y_test = np.array([0] * indices.shape[0] + [1] * indices.shape[0])

X_train = np.cumsum(X_train, axis=1)
X_test = np.cumsum(X_test, axis=1)

train_metric = lambda spike_train1, spike_train2: spknn.distance(spike_train1, spike_train2, metric='isi')
